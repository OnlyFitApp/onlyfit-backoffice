...
const defaultAmbassadorPreparationReason = 'Participante da rede de pré-lançamento; documento fiscal será exigido antes de pagamentos.';
...
function assignmentDraft(item?: AmbassadorAssignment): AssignmentDraft {
  return item ? {
    ...
    displayOrder: String(item.displayOrder), headline: item.headline ?? '',
    ...
    publicVisible: true, displayOrder: '0', headline: '', contractReference: '', startsAt: '', endsAt: '',
  };
}
    ...
    <>
      <header className="page-header ambassador-header">
        <div>
          ...
          <span>Embaixadores e Associados por vertical e país.</span>
        </div>
...
function NetworkTab({ snapshot, filters, setFilters, principals, orphanAssociates, selected, setSelectedId, canEdit, isSuperAdmin, assignmentEditor, setAssignmentEditor, candidateQuery, setCandidateQuery, candidates, candidatesLoading, candidatesError, begin }: {
  ...
  const saveAssignment = async (event: FormEvent) => {
    ...
    const succeeded = await begin(async () => {
      ...
      const persisted = await save.mutateAsync({
        ...
        headline: assignmentEditor.headline,
      ...
      ? `${ambassadorRoleLabel(assignmentEditor.role)} ativado${assignmentEditor.publicVisible ? ' e publicado no app' : ''}.`
  ...
  const publishAssignment = (item: AmbassadorAssignment) => {
    setAction({
      ...
      description: `O ${ambassadorRoleLabel(item.role)} já está ativo e passará a aparecer imediatamente nas áreas públicas do aplicativo.`,
      ...
      run: () => save.mutateAsync({
        ...
        displayOrder: item.displayOrder, headline: item.headline ?? '',
        contractReference: item.contractReference ?? '', startsAt: item.startsAt, endsAt: item.endsAt,
      }),
      success: `${ambassadorRoleLabel(item.role)} publicado no aplicativo.`,
    });
  };
    ...
    <div className="ambassador-network-layout">
      <div className="ambassador-tree-panel">
        ...
        <div className="ambassador-tree">
          ...
          {!snapshot.assignments.length ? <div className="ambassador-empty"><UsersRound size={28} /><strong>Nenhuma atribuição encontrada</strong><span>Cadastre uma região e escolha os primeiros participantes.</span></div> : null}
        </div>
      ...
      {assignmentEditor ? (
        <form className="ambassador-editor" onSubmit={saveAssignment}>
          ...
          <div className="ambassador-form-grid">
            <label className="ambassador-field"><span>Papel</span><select value={assignmentEditor.role} onChange={(event) => setAssignmentEditor({ ...assignmentEditor, role: event.target.value as AssignmentDraft['role'], principalAssignmentId: '' })}><option value="principal">Embaixador</option><option value="associate">Associado</option></select></label>
            ...
            {assignmentEditor.role === 'associate' ? <label className="ambassador-field"><span>Supervisão</span><select value={assignmentEditor.principalAssignmentId} onChange={(event) => setAssignmentEditor({ ...assignmentEditor, principalAssignmentId: event.target.value })}><option value="">Direta pela plataforma</option>{compatiblePrincipals.map((item) => <option value={item.id} key={item.id}>Embaixador · {item.profileName}</option>)}</select></label> : null}
          </div>
          <small className="ambassador-muted">Embaixador representa a vertical no país. Associado pode atuar com um Embaixador ou diretamente com a plataforma.</small>
          ...
          <details className="ambassador-progressive"><summary>Personalização pública</summary><div className="ambassador-form-grid"><label className="ambassador-field"><span>Ordem</span><input type="number" value={assignmentEditor.displayOrder} onChange={(event) => setAssignmentEditor({ ...assignmentEditor, displayOrder: event.target.value })} /></label><label className="ambassador-field wide"><span>Chamada</span><input maxLength={160} value={assignmentEditor.headline} onChange={(event) => setAssignmentEditor({ ...assignmentEditor, headline: event.target.value })} /></label></div><small className="ambassador-muted">O selo público é definido automaticamente pelo papel: {ambassadorRoleLabel(assignmentEditor.role)}.</small></details>
        ...
        <aside className="ambassador-detail-panel">
          <div className="ambassador-panel-head"><div><span>{ambassadorRoleLabel(selected.role)}</span><h2>{selected.profileName}</h2><p>{selected.affinityGroupLabel} · {selected.regionCountryCode} · {selected.regionName}</p></div><CloseButton label="Fechar detalhes" onClick={() => setSelectedId(null)} /></div>
          ...
          {selected.status === 'active' && !selected.publicVisible ? <div className="inline-alert danger"><AlertTriangle size={18} /><span>Este {ambassadorRoleLabel(selected.role)} está ativo, mas oculto no aplicativo.</span></div> : null}
...
function AssignmentCard({ item, selected, onSelect }: { item: AmbassadorAssignment; selected: boolean; onSelect: (id: string) => void }) {
  return <button type="button" className={`ambassador-card ${selected ? 'selected' : ''} ${item.status}`} onClick={() => onSelect(item.id)}><span className="ambassador-avatar">{item.avatarUrl ? <img src={item.avatarUrl} alt="" /> : item.role === 'principal' ? <Crown size={18} /> : <UsersRound size={18} />}</span><span className="ambassador-card-copy"><strong>{item.profileName}</strong><small>{ambassadorRoleLabel(item.role)} · {item.affinityGroupLabel} · {item.regionCountryCode} · {item.regionName}</small><em>{item.headline || 'Sem chamada pública'}</em></span><span className={`ambassador-status ${item.status}`}>{item.status === 'active' && !item.publicVisible ? 'Ativo · oculto' : statusLabel[item.status]}</span><ChevronRight size={16} /></button>;
}
...
function RegionsTab({ regions, canEdit, editor, setEditor, begin }: { regions: CommercialRegion[]; canEdit: boolean; editor: RegionDraft | null; setEditor: (value: RegionDraft | null) => void; begin: (operation: () => Promise<unknown>, success: string) => Promise<boolean> }) {
  ...
  return <div className={`ambassador-regions-layout ${editor ? 'has-editor' : ''}`}>
    <div className="ambassador-list-panel">
      <div className="ambassador-section-head"><div><h2>Países e regiões</h2><p>Definem onde cada participante da rede atua.</p></div>{canEdit ? <button className="button primary compact" onClick={() => setEditor(regionDraft())}><Plus size={15} />Nova região</button> : null}</div>
...
function OperationsTab({ snapshot, canEdit, begin }: {
  ...
  const submitRollout = async (event: FormEvent) => {
    ...
    const succeeded = await begin(() => saveRollout.mutateAsync({ name: rolloutName, affinityGroupKey: rolloutAffinity,
      regionId: rolloutRegion, environment: 'sandbox' }), 'Plano de piloto criado em sandbox para o Clube.');
  ...
  return <div className="ambassador-operations">
    ...
    <section className="ambassador-list-panel"><div className="ambassador-section-head"><div><h2>Piloto e rollout</h2><p>O primeiro piloto é restrito a uma vertical, uma região, sandbox e Clube.</p></div></div>
