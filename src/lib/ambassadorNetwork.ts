...
export async function getAmbassadorSnapshot(filters: { affinityGroupKey?: string; regionId?: string; status?: string; limit?: number; offset?: number } = {}): Promise<AmbassadorSnapshot> {
  const { data, error } = await api.staff.rpc('control_get_ambassador_network_snapshot', {
...
export async function searchAmbassadorCandidates(query: string): Promise<AmbassadorCandidate[]> {
  const { data, error } = await api.staff.rpc('control_search_ambassador_candidates', { p_query: query, p_limit: 20 });
  if (error) throw error;
...
export async function prepareAmbassadorCandidate(input: { profileId: string; reason: string; expiresAt?: string }): Promise<void> {
  const { error } = await api.staff.rpc('control_prepare_ambassador_candidate', {
...
export async function saveCommercialRegion(input: RegionInput): Promise<void> {
  const { error } = await api.staff.rpc('control_save_commercial_region', { p_region_id: input.id ?? null, p_name: input.name, p_slug: input.slug, p_scope_type: input.scopeType, p_country_code: input.countryCode, p_state_code: input.stateCode, p_city_name: input.cityName, p_parent_id: input.parentId, p_specificity: input.specificity, p_priority: input.priority, p_expected_updated_at: input.expectedUpdatedAt ?? null });
  if (error) throw error;
}
export async function setCommercialRegionActive(input: { id: string; active: boolean; expectedUpdatedAt: string }): Promise<void> { const { error } = await api.staff.rpc('control_set_commercial_region_active', { p_region_id: input.id, p_active: input.active, p_expected_updated_at: input.expectedUpdatedAt }); if (error) throw error; }
...
export async function listAmbassadorAudit(): Promise<{ items: AmbassadorAuditEntry[]; total: number }> { const { data, error } = await api.staff.rpc('control_list_ambassador_audit', { p_entity_table: null, p_limit: 100, p_offset: 0 }); if (error) throw error; const row = record(data); return { total: number(row.total), items: Array.isArray(row.items) ? row.items.map((value) => { const item = record(value); return { id: String(item.id ?? ''), actorProfileId: stringOrNull(item.actor_profile_id), actorName: String(item.actor_name ?? 'Sistema'), entityTable: String(item.entity_table ?? ''), entityId: String(item.entity_id ?? ''), action: String(item.action ?? 'update') as AmbassadorAuditEntry['action'], beforeState: item.before_state ? record(item.before_state) : null, afterState: item.after_state ? record(item.after_state) : null, occurredAt: String(item.occurred_at ?? '') }; }) : [] }; }

export async function getAmbassadorOperationsDashboard(): Promise<AmbassadorOperationsDashboard> {
  const { data, error } = await api.staff.rpc('control_get_ambassador_operations_dashboard');
  if (error) throw error;
